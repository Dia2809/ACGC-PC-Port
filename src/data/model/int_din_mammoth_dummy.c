...
#include "assets/int_din_mammoth_dummy/int_din_mammoth_pal.inc"
};
#endif

#ifdef TARGET_PC
u8 int_din_mammoth_dummy_tex[0x200];
#else
u8 int_din_mammoth_dummy_tex[] = {
#include "assets/int_din_mammoth_dummy_tex.inc"
};
#endif

#ifdef TARGET_PC
Vtx int_din_mammoth_dummy_v[0x80 / sizeof(Vtx)];
#else
Vtx int_din_mammoth_dummy_v[] = {
#include "assets/int_din_mammoth_dummy_v.inc"
};
#endif

Gfx int_din_mammoth_dummy_model[] = {
    gsSPTexture(0, 0, 0, G_TX_RENDERTILE, G_ON),
    gsDPSetRenderMode(G_RM_FOG_SHADE_A, G_RM_AA_ZB_TEX_EDGE2),
    gsDPSetCombineLERP(TEXEL0, 0, SHADE, 0, 0, 0, 0, TEXEL0, PRIMITIVE, 0, COMBINED, 0, 0, 0, 0, COMBINED),
    gsDPLoadTLUT_Dolphin(15, 16, 1, int_din_mammoth_pal),
    gsDPSetTextureImage_Dolphin(G_IM_FMT_CI, G_IM_SIZ_4b, 32, 32, int_din_mammoth_dummy_tex),
    gsDPSetTile_Dolphin(G_DOLPHIN_TLUT_DEFAULT_MODE, 0, 15, GX_MIRROR, GX_MIRROR, 0, 0),
    gsDPSetPrimColor(0, 128, 255, 255, 255, 255),
    gsSPLoadGeometryMode(G_ZBUFFER | G_SHADE | G_CULL_BACK | G_FOG | G_LIGHTING | G_SHADING_SMOOTH | G_DECAL_LEQUAL),
    gsSPVertex(int_din_mammoth_dummy_v, 8, 0),
